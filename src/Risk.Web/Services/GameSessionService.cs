    ...
    NetworkedSeatContext? net = null)
{
    ...
    /// <summary>
    /// The networked table's bot failure, if a bot turn hard-stopped.
    /// Null in hot-seat (no bots there) and while every bot turn completes.
    /// Read-only passthrough — the session owns it.
    /// </summary>
    public AiTurnFailure? AiFailure => ActiveNetwork?.AiFailure;
