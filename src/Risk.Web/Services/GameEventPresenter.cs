...
{
    public static string Describe(GameEvent gameEvent) => Describe(gameEvent, null);

    public static string Describe(GameEvent gameEvent, Func<PlayerId, string?>? nameOf)
    {
        string Label(PlayerId id) => PlayerLabel(id, nameOf);
        return gameEvent switch
        {
            TerritoriesAssigned => "Los territorios fueron repartidos entre los jugadores.",
            NeutralTroopsPlaced e => $"{Label(e.Placer)} colocó {e.Troops} tropas neutrales en {e.Territory.Value}.",
            TerritoryClaimed e => $"{Label(e.Player)} reclamó {e.Territory.Value}.",
            TroopsPlaced e => $"{Label(e.Player)} colocó {e.Troops} tropas en {e.Territory.Value}.",
            CardsTraded e => e.BonusTerritory is { } bonusTerritory
                ? $"{Label(e.Actor)} canjeó {e.Cards.Count} cartas por {e.Bonus} tropas y +2 tropas extra en {bonusTerritory.Value}."
                : $"{Label(e.Actor)} canjeó {e.Cards.Count} cartas por {e.Bonus} tropas.",
            BattleResolved e =>
                $"{Label(e.Attacker)} atacó {e.To.Value} desde {e.From.Value}: perdió {e.AttackerLosses} tropas, el defensor perdió {e.DefenderLosses}.",
            TerritoryConquered e => $"{Label(e.Conqueror)} conquistó {e.Territory.Value}.",
            TerritoryOccupied e => $"{Label(e.Player)} ocupó {e.Territory.Value} con {e.Troops} tropas.",
            TroopsFortified e => $"{Label(e.Actor)} movió {e.Troops} tropas de {e.From.Value} a {e.To.Value}.",
            PlayerEliminated e => $"{Label(e.Victim)} fue eliminado por {Label(e.By)} y le cedió {e.CardsTransferred} cartas.",
            ...
            HeadquartersCaptured e => $"{Label(e.Attacker)} capturó el Cuartel General de {Label(e.OriginalOwner)} en {e.Territory.Value}.",
            _ => "Ocurrió un evento."
        };
    }
    ...
    /// </summary>
    private static string PlayerLabel(PlayerId id, Func<PlayerId, string?>? nameOf = null)
    {
        var name = nameOf?.Invoke(id);
        return string.IsNullOrWhiteSpace(name) ? $"Jugador {id.Value + 1}" : name;
    }
}
