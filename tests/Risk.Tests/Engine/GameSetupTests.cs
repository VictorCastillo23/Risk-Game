...
public class GameSetupTests
{
    ...
    [Fact]
    public void Create_deals_all_42_territories_equitably_across_2_players()
    {
        // TwoPlayer is now the sole mode still on the shared upfront
        // random-deal branch: Classic moved to the unclaimed Claim-phase
        // start in 2.1, and Capital joined it in 5.1 (see
        // Create_starts_Classic_and_Capital_with_all_territories_unclaimed_...
        // above). 42 splits evenly across 2 players, so both counts are
        // exactly 21 — no remainder to spread unevenly.
        var ok = Assert.IsType<CommandResult<GameState, GameEvent>.Ok>(
            GameSetup.Create(2, GameMode.TwoPlayer, QueuedDiceRoller.ForRollOff(2)));
        ...
        Assert.All(counts.Values, count => Assert.Equal(21, count));
    }
