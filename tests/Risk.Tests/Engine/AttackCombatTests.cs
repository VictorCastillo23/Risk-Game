...
public class AttackCombatTests
{
    ...
    [Fact]
    public void Execute_rejects_an_attack_against_an_unclaimed_defender_territory()
    {
        var attacker = new PlayerId(0);
        var state = BuildUnclaimedDefenderState(attacker, Alaska, 4, NorthwestTerritory);
        var engine = new GameEngine(new QueuedDiceRoller());
    ...
    /// <summary>
    /// Builds a minimal Attack-phase state where <paramref name="to"/> has no
    /// owner (<c>Owner == null</c>), so <c>ExecuteAttack</c>'s defender guard
    /// must reject it without ever reaching the <c>.Value</c> unwraps in
    /// <c>TerritoryConquered</c>/<c>EliminatePlayer</c>.
    /// </summary>
    private static GameState BuildUnclaimedDefenderState(PlayerId currentPlayer, TerritoryId from, int fromTroops, TerritoryId unclaimedTo)
    {
        ...
        foreach (var territory in WorldMap.Territories)
        {
            territories[territory.Id] = territory.Id == from
                ...
                : territory.Id == unclaimedTo
                    ? new TerritoryState(null, 3)
        ...
        [
            ...
            new PlayerState(new PlayerId(currentPlayer.Value + 1), [], false, 0)
        ];

        var turn = new TurnState(currentPlayer, TurnPhase.Attack);
