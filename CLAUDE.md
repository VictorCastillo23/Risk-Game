...
`Risk.Domain` + `Risk.Engine` (+ `Risk.Tests`) is the headless rules engine, and `Risk.Web` is a playable Blazor Server UI built on top of it (hot-seat: all players share one browser tab, turn by turn). `Risk.AI` is a deterministic heuristic bot, built specifically to prove that `Risk.Engine`'s public API (`IGameEngine.Observe`/`PlayerView`) is a sufficient, clean contract for a non-human client with no access to hidden information — like `Risk.Web`, it never reads `GameState` directly, only the redacted `PlayerView`. It is not yet wired into `Risk.Web`; a bot-controlled seat in the playable UI is a future change.
...
- **`Risk.AI`** (`src/Risk.AI`) — a deterministic heuristic bot (`Risk.Domain` + `Risk.Engine` dependencies only, same as `Risk.Web`). `IBotPlayer.DecideNextCommand(PlayerView, BotMemory)` is a pure function returning the next `GameCommand` plus an updated `BotMemory` — a small immutable record carrying only state legitimately derivable from the bot's own past actions (its running `ReinforcePool`, its own Setup-phase troop count, which `PlayerId`s it has observed as `Turn.CurrentPlayer`), never from hidden `GameState`. `BotPlayer.cs` dispatches by state in the same order `GameEngine.Execute` validates it (pending occupation → mandatory trade-down → phase switch) to one of the seven `Decisions/*` classes (`Claim`, `Setup`, `Headquarters`, `Reinforce`, `Attack`, `Occupy`, `Fortify`), each scoring its legal options via `Scoring/*` (`TerritoryScoring` for frontier/threat assessment, `CombatOdds` for a closed-form dice-outcome expectation table, `MissionScoring`/`CapitalScoring` for mission- and Capital-mode-aware weighting that is a no-op in Classic mode, `BotWeights` centralizing every tuning constant). `BotTurnRunner`/`BotTurnStep` orchestrate the `Observe → Decide → Execute` loop and enforce this project's one hard, non-negotiable guarantee: a `Rejected` from the engine is never masked by a fallback command — it surfaces as an unmistakable `BotRunResult.Rejected` (a closed hierarchy alongside `Completed`/`Exhausted`). `tests/Risk.AI.Tests/Engine/BotVsBotFullGameIntegrationTests.cs` proves this guarantee end-to-end: full bot-vs-bot games for all four `GameMode`s (Classic, SecretMission, Capital, TwoPlayer) reaching a valid win with zero illegal commands ever issued.

## Conventions specific to this repo

- **Strict TDD is mandatory** for engine/domain/AI work: write the failing xUnit test first, confirm red, then implement. This has been followed consistently in the existing code — don't break the pattern.
...
- All rule validation belongs in `Risk.Engine`, never in `Risk.Domain`, `Risk.Web`, or `Risk.AI` — this was an explicit architecture decision resolving a tension between where rules "live" (Domain, per the project's description) and where validation "lives" (Engine, per its own architecture requirements). `Risk.AI` deciding via anything other than `PlayerView` (e.g. reading raw `GameState`) would be an architecture violation, not just a style issue — it's the one thing the whole project exists to prove doesn't happen.
